...
export default function App() {
  ...
  // Function to logout/reset
  const handleLogout = () => setUserRole(null);

  // 1. If no role is selected, show the Landing Page (General Access)
  if (!userRole) {
    return <LandingPage onSelectRole={setUserRole} />;
  }

  // 2. If Teacher is selected, show the Teacher Portal file
  if (userRole === 'teacher') {
    return <TeacherPortal onLogout={handleLogout} />;
  }

  // 3. If Student is selected, show the Student Portal file
